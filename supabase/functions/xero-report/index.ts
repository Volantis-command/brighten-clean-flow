  ...
  try {
    ...
    if (expiresAt <= new Date(Date.now() + 60000)) {
      ...
      if (!clientId || !clientSecret) {
        return new Response(JSON.stringify({ error: "XERO_CLIENT_ID or XERO_CLIENT_SECRET not set" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      ...
      if (!refreshRes.ok) {
        return new Response(JSON.stringify({ error: "Token refresh failed", detail: await refreshRes.text() }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      const refreshed = await refreshRes.json();
      accessToken = refreshed.access_token;
      await supabase.from("xero_tokens").update({ access_token: refreshed.access_token, refresh_token: refreshed.refresh_token, expires_at: new Date(Date.now() + refreshed.expires_in * 1000).toISOString() }).eq("id", tokenData.id);
    }

    const params = new URLSearchParams();
    if (from_date) params.set("fromDate", from_date);
    if (to_date) params.set("toDate", to_date);
    const url = XERO_API_BASE + "/" + REPORT_ENDPOINTS[report_type] + (params.toString() ? "?" + params.toString() : "");

    const xeroRes = await fetch(url, { headers: { Authorization: "Bearer " + accessToken, "Xero-Tenant-Id": XERO_TENANT_ID, Accept: "application/json" } });
    const xeroData = await xeroRes.json();
    if (!xeroRes.ok) {
      return new Response(JSON.stringify({ error: "Xero API error", status: xeroRes.status, detail: xeroData }), { status: xeroRes.status, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
