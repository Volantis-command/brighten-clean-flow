    ...
    try {
      ...
      const { data: bookingResult, error: bookingError } = await supabase.functions.invoke(
        'create-booking-from-quote',
        {
          body: {
            property_id: null,
            preferred_date: quote.preferred_date || new Date().toISOString().split('T')[0],
            preferred_time: quote.preferred_time?.includes('Morning') ? '08:00' : quote.preferred_time?.includes('Afternoon') ? '13:00' : null,
